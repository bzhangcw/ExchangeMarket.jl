...
function __linear_grad_fromx!(alg, fisher::FisherMarket)
    alg.∇ .= fisher.q - sum(fisher.x; dims=1)[:]
end
...
function __linear_hess_fromx!(alg, fisher::FisherMarket; bool_dbg=false)
    ...
    Di(i) = begin
        ...
        c = fisher.val_∇u[i, :]
        w = fisher.w[i]
        jxp = __linear_jacxp(X₂, u, c, w, alg.μ)
        if bool_dbg
            Xi₂ = spdiagm(1 ./ X2[i, :])
            jpx = __linear_jacpx(Xi₂, u, c, w, alg.μ)
...
function __ces_hess_dual!(alg, fisher::FisherMarket)
    ...
    _Hi = (i) -> begin
        _H = spdiagm(1 / fisher.val_ν[i] .* fisher.val_Hν[i, :]) -
             (1 / fisher.val_ν[i])^2 * fisher.val_∇ν[i, :] * fisher.val_∇ν[i, :]'
        return _H .* (w[i] / σ)
    end
    alg.H .= mapreduce(_Hi, +, 1:fisher.m, init=spzeros(fisher.n, fisher.n))
end
...
function __ces_eval_dual!(alg, fisher::FisherMarket)
    ...
    alg.φ = (
        alg.p' * fisher.q -
        alg.μ * logbar(alg.p) +
        sum((w[i] / σ) * log(fisher.val_ν[i]) for i in 1:fisher.m)
    )
end
